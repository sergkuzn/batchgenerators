...
def producer_fixmatch(queue, data_loader, transform, transform_weak, thread_id, seed, abort_event,
            ...
            if not abort_event.is_set():
                ...
                if item is None:
                    try:
                        item = next(data_loader)
                        if transform is not None:
                            item = [transform(**item), transform_weak(**item)]
                    except StopIteration:
                        item = "end"
...
class MultiThreadedAugmenterFixMatch(object):
    """ Makes your pipeline multi threaded. Yeah!
    If seeded we guarantee that batches are retunred in the same order and with the same augmentation every time this
    is run. This is realized internally by using une queue per worker and querying the queues one ofter the other.
    Args:
    ...
    def __init__(self, data_loader, transform, transform_weak, num_processes, num_cached_per_queue=2, seeds=None,
                 pin_memory=False, timeout=10, wait_time=0.02):
        self.timeout = timeout
        self.pin_memory = pin_memory
        self.transform = transform
    ...
    def _start(self):
        if not self.was_initialized:
            ...
            with threadpool_limits(limits=1, user_api="blas"):
                for i in range(self.num_processes):
                    self._queues.append(Queue(self.num_cached_per_queue))
                    self._processes.append(Process(target=producer_fixmatch, args=(
                        self._queues[i], self.generator, self.transform, self.transform_weak, i, self.seeds[i],
                        self.abort_event)))
                    self._processes[-1].daemon = True
                    self._processes[-1].start()
